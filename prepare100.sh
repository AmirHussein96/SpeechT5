...
if [ $stage -le 0 ] && [ $stop_stage -ge 0 ]; then
    ...
    # Rename the file for training
    cp ${tsv_dir}/train${train_suf}/train.tsv ${tsv_dir}/speech_train${train_suf}.tsv
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
    ...
    python SpeechT5/SpeechT5/scripts/generate_dict.py \
        -i ${lab_dir}/speech_train${train_suf}.km \
...
if [ $stage -le 6 ] && [ $stop_stage -ge 6 ]; then
    python SpeechT5/SpeechT5/fairseq/examples/wav2vec/libri_labels.py $tsv_dir/speech_train100.tsv --output-dir $tsv_dir --output-name train100_trans
...
for split in "speech_valid" "dev_clean" "dev_other" "test_clean" "test_other"; do
        python SpeechT5/SpeechT5/fairseq/examples/wav2vec/libri_labels.py $tsv_dir/speech_valid.tsv --output-dir $tsv_dir --output-name speech_valid
    done
if [ $stage -le 7 ] && [ $stop_stage -ge 7 ]; then
    log "Stage 6: Finalize the pretrain data folder..."

    pretrain_data_dir=${data_dir}/pretrain100
