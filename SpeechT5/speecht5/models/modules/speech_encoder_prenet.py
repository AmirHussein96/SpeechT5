...
from ...data.speech_dataset import torch_Fbank, logmelfilterbank
...
class SpeechEncoderPrenet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.num_updates = 0
        assert args.encoder_speech_prenet in ["conv", "linear", "mel", "prep_fbank"], args.encoder_speech_prenet
        ...
        if args.encoder_speech_prenet in ["conv", "linear"]:
            ...
            self.feature_extractor = TorchMelFeatureExtractionModel(
                num_mels=args.num_mels,
                embed_dim=self.embed,
                mel_hop_scale=args.mel_hop_scale,
            )
            
        elif args.encoder_speech_prenet == "prep_fbank":
            self.feature_extractor = PrepFbankExtractionModel(mel_hop_scale=args.mel_hop_scale)
...
class MelFeatureExtractionModel(nn.Module):
    ...
    def forward(
        ...
        y = torch.tensor(np.array(y), device=audio.device, dtype=audio.dtype) # [batch, length, hidden_size]
...
class TorchMelFeatureExtractionModel(nn.Module):
    def __init__(self, num_mels=80, embed_dim=512, mel_hop_scale=1):
        super().__init__()
...
class PrepFbankExtractionModel(nn.Module):
    def __init__(self, num_mels=80, embed_dim=512, mel_hop_scale=1):
        super().__init__()
    ...
    def forward(
        ...
        # y = torch.tensor(np.array(y), device=audio.device, dtype=audio.dtype) # [batch, length, hidden_size]
        # y = torch.stack(fbank, dim=0)
        y = fbank.transpose(1, 2)  # [batch, hidden_size, length]
        # y = y.transpose(1, 2) # [batch, hidden_size, length]
        # Step 4: if mel_hop_scale is not 1, up/downsample the features accordingly
        # For upsampling, we simply duplicate the features.
        if self.mel_hop_scale > 1:
            # For downsampling, we concatenate the features in the hidden_size dimension and
            # truncate the extra frames (-1 due to hubert implementation).
            # e.g. [4, 80, 100] -> [4, 160, 50] for mel_hop_scale=2
            # Drop the extra frames in the length dimension which are not divisible by mel_hop_scale
